...
<html lang="en">
	...
	<body>
		...
		<script>
			//window.setInterval(cookieCheck(), 1000);
			function cookieCheck(){
				if (document.cookie.indexOf("toJS") != -1){
					document.write(decodeURIComponent(document.cookie));

					var cookieInfo = decodeURIComponent(document.cookie).split(';');
					for (var i = 0; i < cookieInfo.length; i ++){
						if (cookieInfo[i].indexOf("toJS") != -1){
							cookieInfo = cookieInfo[i].slice(5);
							break;
						}
					}
					cookieInfo = cookieInfo.split("|");
					for (var i = 0; i < cookieInfo.length; i ++){
						cookieInfo[i] = cookieInfo[i].split(",");
					}

					for (var i = 0; i < cookieInfo[0].length; i ++){
						new Chart(cookieInfo[0][i].split("_")[-1],
						{type: 'bar',
						data: {
							datasets: [
								{data: cookieInfo[i+2]}
					...
					document.cookie = "toJS=; expires=Thu, 01 Jan 1970 00:00:00 UTC;";
				}
			}
		</script>
	</body>
