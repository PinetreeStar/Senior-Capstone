...
function loginVal () {
    ...
    xhttp.open("GET", "TestingPHP.php?q="+user+"|"+pass);
    xhttp.send();
...
function createTimeBox(data, index) {
    if (index == 0){
        return `
        <input type="radio" id="${data[0]}" name="timeOption">
        <label for="${data[0]}">${data[0]}: <input type="text" id="${data[0]}:inp" placeholder="MMDDYYYY"></label><br>
        `;
    }else{
        return `
        <input type="radio" id="${data[0]}" name="timeOption" value="${data[1]}">
...
function queryData() {
    ...
    xhttp.open("GET", "TestingPHP2.php?q=" + toPHP);
    xhttp.send();
